...
- **Acrylicの幅波打ち回帰はcoalesced入力の再提示が根因候補（2026-08-25、実機gate待ち）**: Apple Pencilの失敗strokeには、`37792, 37796, 37792, 37796`のようにSafariが直前の`getCoalescedEvents()`群を次callbackでも再提示した痕跡が大量にあった。旧samplingは距離があれば古いtimestampも採用したため、経路が周期的に後退・再前進して輪郭が波打った。WebKitで300点の各batchを二重提示すると修正前は599点を採用し、幅標準偏差は`1.513 / 1.442 / 1.456px`、古いtimestampと同時刻・同座標の再提示を拒否すると300点・`1.156 / 1.004 / 1.000px`へ戻った。`PressureDynamics.smoothingMs=50`は別の候補対策として残るが、この入力巻き戻しを隠す用途にはしない。最終gateは新captureの採用timestampが単調であることとApple Pencilの見た目確認。実機確認までは修正済み扱いにしない。
