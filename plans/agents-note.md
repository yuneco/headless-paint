...
- **Acrylic WebGL2の残課題（2026-08-30更新）**: `BrushAccelerator`、reactの`gpuBackend`、appsのreload切替まで正式APIへ統合済み。context loss中のstrokeは、residency hitではcommit直前に退避したdirty領域、missでは既存sampling snapshotへ戻して全入力をCPU再実行し、以後もCPUへ固定する。iOS WebKit向けにcommit passごとの`transferToImageBitmap` 1回化、筆圧上限footprintによるsnapshot arrayの安定確保、GPU resourceの非縮小再利用を追加した。Mac WebKit backlogとiPad実機の`bitmapMs` / `drawMs`、長時間stroke・cold first stroke、dirty退避のdraw回数・面積とlazy確保時のメモリpressureは引き続き確認する。`packages/engine/docs/gpu-acceleration.md`の「GPU checkpointはCPUと同じ原点・寸法」という記述は、GPUだけ最大footprint tileを使う新仕様との更新差分が残る（今回ユーザー指定によりdocs未編集）。
