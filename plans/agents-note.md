...
- **Acrylic G1 WebGL2 spikeの検証残件（2026-08-29更新）**: full-layer RGBA8 accum、instanced dab、18×8 field texture、固定512×512 commit framebuffer、pointer batch単位のasync checkpointをexperiment-only runtime scratchで接続した。commitは512×512 tileへ分割し、async readbackもbatch dirty rect（超過時は最新dab側512×512）へ集約済み。Chromium browser testで連続commit保持、512px超dirty rectのlayer全域一致、async矩形を確認した。次はWebKit/Chromium/STPで`gpuCommit`・`gpuReadRequest`・dispatchを再計測し、full-layer stroke開始uploadの削減要否を判断する。正式化前にfractional checkpoint originとCPU/GPU Tier B差分も確認する。Expand、source-over以外、alpha lock、context loss復旧は未対応。
