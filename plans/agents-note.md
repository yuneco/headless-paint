...
- **Rough bristle C4 fused CPU rasterはbrowser parity / 性能gate待ち（2026-08-25）**: mixing OFF限定の実験経路を`brushPerfDebug.experiments.fusedInk`（既定OFF、`?fusedInk=1`）で追加した。mask triangle走査の横断`v`から、既存lane規則で事前rasterした1D profileを線形参照し、`max(maskAlpha * inkAlpha)`で着色済みImageDataを作る。これにより局所ink canvas、segmentごとの`setTransform` + atlas `drawImage`、`source-in`着色、`destination-in` mask合成を省略する。S字 / 反復交差60pxのTier Bテストは追加済みだが、sandboxではbrowser Vitestが待受権限で起動できないため、metric合否とE1bの採否は検収側browser実行・官能評価・production benchmark後に判断する。
