...
- **Acrylic WebGL2の残課題（2026-08-30更新）**: `BrushAccelerator`、reactの`gpuBackend`、appsのreload切替まで正式APIへ統合済み。context loss中のstrokeは、residency hitではcommit直前に退避したdirty領域、missでは既存sampling snapshotへ戻して全入力をCPU再実行し、以後もCPUへ固定する。rollback canvasは初回dirty退避時にlayer同寸で確保して同寸stroke間で再利用するため、iPad Safari実機gateでは長時間stroke・cold first strokeに加え、dirty退避のdraw回数・面積とlazy確保時のメモリpressureを確認する。
