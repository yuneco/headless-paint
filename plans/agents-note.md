...
- **Acrylic G1 WebGL2 spikeの検証残件（2026-08-29）**: full-layer RGBA8 accum、instanced dab、18×8 field texture、同期checkpoint readback、pointer batch単位commitをexperiment-only runtime scratchで接続した。Chromium browser testでsource upload/readbackのunpremultiplyとdab commitは通過。正式化前にfractional checkpoint origin（同期`readPixels`版は包含pixelへfloor）とCPU/GPU Tier B差分を確認する。Expand、source-over以外、alpha lock、context loss復旧、async readbackは未対応。
