...
- **Acrylic WebGL2の残課題（2026-08-30更新）**: `BrushAccelerator`、reactの`gpuBackend`、appsのreload切替まで正式APIへ統合済み。context loss中のstrokeは開始時snapshotへ戻して全入力をCPU再実行し、以後もCPUへ固定する。確実なrollbackのためGPU strokeごとにlayer 1枚分のCanvas2D snapshotを保持するので、iPad Safari実機gateでは長時間stroke・cold first strokeに加えてsnapshotの時間/メモリ費用を確認する。問題が顕在化した場合だけdirty領域snapshotを別設計する。
