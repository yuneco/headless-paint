...
- **Acrylic G1 WebGL2 spikeの検証残件（2026-08-29更新）**: full-layer RGBA8 accum、instanced dab、18×8 field texture、固定512×512 commit framebuffer、pointer batch単位のasync checkpointをexperiment-only runtime scratchで接続した。commitは512×512 tileへ分割する。async readbackを前batchのdirty rectだけにすると、1 batch遅延後のsampling footprintがtile外になりpickupが全消失したため、最新dab中心の最大512×512へ修正した。Chromium browser testでdocument座標とunpremultiplied source色、completed tile取り込み後のfield pickupを確認済み。次はWebKit/Chromium/STPでparity、`gpuCommit`・`gpuReadRequest`・dispatchを再計測し、固定tile拡大の転送costとfull-layer stroke開始uploadの削減要否を判断する。正式化前にfractional checkpoint originとCPU/GPU Tier B差分も確認する。Expand、source-over以外、alpha lock、context loss復旧は未対応。
