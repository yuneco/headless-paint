...
- **Rough bristleのLab textureはprocedural Fine tooth（2026-08-22）**: 外部画像assetではなく、2周波value noiseによるdocument-spaceの高さ場（scale 4px / amount 0.85 / hardness 0.82 / seed 1）だった。同じ式をengineへ移植し、筆圧16段階では接触maskだけを更新する。追加ライセンスはない。TEX-03で収集した外部CC0画像はOrganic sponge向け監査素材であり、Roughのreferenceとしてproductionへ入れない。
