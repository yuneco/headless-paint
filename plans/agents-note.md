...
- **Acrylicの幅波打ち回帰は未解決（2026-08-24、再調査中）**: `PressureDynamics.smoothingMs=50`はWebKit固定300点fixtureの短周期筆圧波形を減衰させたが、Apple Pencil官能gateでは不要な幅の膨縮が残った。したがって因果平滑化は候補対策であり、修正済みとは扱わない。合成水平線の幅統計だけでは、実座標への`causal-adaptive`、coalesced callback境界、raw pressureを使うeffective-size spacingの影響を切り分けられない。Productionで失敗する次の1strokeをbrush設定・filter・callback batch境界付きJSONとして採取し、同じ入力を段階比較してから根因を確定する。最終gateは実入力固定replayとApple Pencil再確認の両方とする。
