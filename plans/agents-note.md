...
- **Rough bristleのLab textureはprocedural Fine tooth（2026-08-22）**: 外部画像assetではなく、2周波value noiseによるdocument-spaceの高さ場だった。Lab参照値はscale 4px / amount 0.85 / hardness 0.82 / seed 1、Production既定値は官能評価によりscale 4px / amount 1 / hardness（UI上のContrast）0.75 / seed 1へ決定した。同じ式をengineへ移植し、筆圧16段階では接触maskだけを更新する。追加ライセンスはない。TEX-03で収集した外部CC0画像はOrganic sponge向け監査素材であり、Roughのreferenceとしてproductionへ入れない。
...
- **Rough bristle production初期版は不透明paintを対象とする（2026-08-22）**: 連続掃引chunkは小さく重ねて継ぎ目を防ぐため、半透明paintでは重なり濃度が見える可能性がある。反復接触は追加pigment canvasを持たず、面掠れで着彩可能な領域の低alpha紙目をsource-overで蓄積する簡素なモデル。半透明対応や厳密な顔料厚は必要性を再評価してから別設計にする。
- **Rough bristleの面掠れは最終pixelで判定する（2026-08-22）**: Lab COMBは低解像度cellを符号付きpaint fieldのままswept quadへbilinear補間し、最終pixelでhardnessを適用、重複quadを`max(alpha)`結合していた。alpha atlasを先に作って区間ごとにCanvas `source-over`すると、低筆圧の0/1境界へ薄いalphaが蓄積して全面着彩になる。productionもsoftware raster順序へ統一し、固定S字のWebKit Call p50 / p95 `1 / 10ms`を確認した。初回未着彩cellのfloorと専用反復強度は持たない。
- **紙目のpixel-local pressure fieldはRough専用にしない（2026-08-22）**: Roughのstroke-space面掠れは各点の筆圧を補間済みだが、後段のdocument-space surface grainだけは描画chunkの平均筆圧を16段階化している。局所筆圧で紙目への接触率を変える仕組みは鉛筆・パステル等にも共通するため、Rough統合へ専用fieldを追加せず、共通Surface grain機構の別作業として設計する。現在の固定S字で薄い全面着彩になった主因はこの近似ではなくalpha評価順だった。
