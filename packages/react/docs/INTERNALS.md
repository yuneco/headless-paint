...
### 2. structural コマンド（add-layer / remove-layer / reorder-layer / duplicate-layer / merge-layer-down）

レイヤー構造の変更を元に戻す/やり直す:

- **add-layer Undo**: レイヤーを削除
- **remove-layer Undo**: レイヤーを再挿入し、`rebuildLayerFromHistory` で削除直前 checkpoint から復元する
- **reorder-layer Undo/Redo**: 逆/順方向に移動
- **duplicate-layer Undo**: 複製先レイヤーを削除する
- **duplicate-layer Redo**: `applyDuplicateLayerCommand` で recorded topology に従って複製し、複製先を active にする
- **merge-layer-down Undo**: source layer を元の位置へ復元し、source / target を `rebuildLayerFromHistory` で復元する
- **merge-layer-down Redo**: `applyMergeLayerDownCommand` で recorded topology に従って source を target へ焼き込み、target を active にする

`removeLayer` 時は削除対象、`duplicateLayer` 時は source、`mergeLayerDown` 時は source / target を `beginHistoryMutation` に渡す。`pushCommand` は checkpoint coverage を検証し、coverage がない command は undoable 履歴へ追加しない。
