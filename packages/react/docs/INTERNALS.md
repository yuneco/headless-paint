...
| レイヤー操作の履歴連携 | addLayer / removeLayer / moveLayerUp / moveLayerDown の各操作で対応するコマンドを作成。remove-layer は削除直前に `beginHistoryMutation` |
...
`removeLayer` 時は `pushCommand` ではなく、削除直前の `beginHistoryMutation` が checkpoint を作る。`pushCommand` は checkpoint coverage を検証し、coverage がない command は undoable 履歴へ追加しない。
