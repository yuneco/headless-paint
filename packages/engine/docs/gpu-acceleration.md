...
- **checkpoint snapshot**: `checkpointDistancePx` ごとに、accum の局所 tile（CPU の `captureCheckpoint` と同じ原点・寸法）を branch 別の snapshot texture へ GPU 内で blit する。field の sampling 元はこの snapshot であり、CPU 経路の「直近 checkpoint 時点の tile を読む」時間基準を再現する
- **commit**: pointer batch ごとに 1 回、branch 別 dirty rect を commit canvas（1024²）へ敷き詰めて一括 blit し、`layer.ctx.drawImage` で書き戻す。dab ごとや点ごとには書き戻さない
